...
//	int qflag = 0;
	...
	// compute the options
	while ((c = getopt (argc, argv, "e:f:k:d:w:lphFEA")) != -1)
		switch (c)
		{
			case 'd':
				dvalue = optarg;
...
void print_help()
{
	...
	printf("   -d curl -> use curl for downloading the file\n");
	printf("      usage of -d curl in combination with -f\n");
